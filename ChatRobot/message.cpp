...
QNChatMessage::QNChatMessage(QWidget*parent) : QWidget(parent){
    ...
    te_font.setFamily("MicrosoftJhengHei");
    te_font.setPointSize(12);
    this->setFont(te_font);
    m_leftPixmap = QPixmap("/home/alientek/GPT.jpg");
    m_rightPixmap = QPixmap("/home/alientek/User.jpg");
    m_loadingMovie = new QMovie(this);
    m_loadingMovie->setFileName("/home/alientek/loading.gif");
...
void QNChatMessage::setText(QString text, QString time, QSize allSize, User_Type userType){
    m_msg = text;
    m_userType = userType;
    m_time = time;
    m_curTime = QDateTime::currentDateTime().toString("hh:mm");
    m_allSize = allSize;
    if(!m_isSending){
        m_loading->move(m_kuangLeftRect.x()-m_loading->width()-10, m_kuangRightRect.y()+m_kuangRightRect.height()/2-m_loading->height()/2);
        m_loading->show();
...
QSize QNChatMessage::fontRect(QString str){
    ...
    m_iconLeftRect = QRect(iconSpaceW, inconTMPH, iconWH, iconWH);
    m_iconRightRect = QRect(this->width()-iconSpaceW-iconWH, inconTMPH, iconWH, iconWH);
    ...
    m_textLeftRect.setRect(m_kuangLeftRect.x()+textSpaceRect,m_kuangLeftRect.y()+inconTMPH,
                           m_kuangLeftRect.width()-2*textSpaceRect,m_kuangLeftRect.height()-2*inconTMPH);
    m_textRightRect.setRect(m_kuangRightRect.x()+textSpaceRect,m_kuangRightRect.y()+inconTMPH,
                            m_kuangRightRect.width()-2*textSpaceRect,m_kuangRightRect.height()-2*inconTMPH);
